...
-export([dupes_test/1, join_test/1, adr_test/1]).
...
join_test(Config) ->
    ...
    {_NetID, _DevAddr, _DLSettings, _RxDelay, NwkSKey, AppSKey} =
        test_utils:wait_for_join_resp(PubKeyBin1, AppKey, JoinNonce),
...
adr_test(Config) ->
    ...
    receive
        {client_data, _, _Data3} ->
            Decoded =
                {response, Response} = blockchain_state_channel_message_v1:decode(_Data3),
