...
mqtt_test(Config) ->
    ...
    {ok, _, _} = emqtt:subscribe(
        MQTTConn,
        <<SubTopic/binary, "helium/", ?CONSOLE_DEVICE_ID/binary, "/rx">>,
        0
    ),
    ...
    emqtt:publish(
        MQTTConn,
        <<SubTopic/binary, "helium/", ?CONSOLE_DEVICE_ID/binary, "/tx/channel">>,
...
mqtt_update_test(Config) ->
    ...
    {ok, _, _} = emqtt:subscribe(
        MQTTConn,
        <<SubTopic0/binary, "helium/", ?CONSOLE_DEVICE_ID/binary, "/rx">>,
        0
    ),
    ...
    receive
        {publish,
            #{payload := Payload0, topic := <<SubTopic0:5/binary, _/binary>>}} ->
            self() ! {channel_data, jsx:decode(Payload0, [return_maps])}
    ...
    {ok, _, _} = emqtt:subscribe(
        MQTTConn,
        <<SubTopic1/binary, "helium/", ?CONSOLE_DEVICE_ID/binary, "/rx">>,
        0
    ),
    ...
    MQTTChannel1 = #{
        <<"type">> => <<"mqtt">>,
        <<"credentials">> =>
            #{<<"endpoint">> => <<"mqtt://127.0.0.1:1883">>, <<"topic">> => SubTopic0},
        <<"id">> => ?CONSOLE_MQTT_CHANNEL_ID,
        <<"name">> => ?CONSOLE_MQTT_CHANNEL_NAME
    },
    ets:insert(Tab, {channels, [MQTTChannel1]}),
    {ok, WorkerPid} =
        router_devices_sup:maybe_start_worker(
            WorkerID,
            #{}
        ),
    {ok, _, _} = emqtt:unsubscribe(
        MQTTConn,
        <<SubTopic1/binary, "helium/",
            ?CONSOLE_DEVICE_ID/binary,
            "/rx">>
    ),
    {ok, _, _} = emqtt:subscribe(
        MQTTConn,
        <<SubTopic0/binary, "helium/",
            ?CONSOLE_DEVICE_ID/binary,
            "/rx">>,
        0
    ),
