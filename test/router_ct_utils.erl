...
stop_miners(Miners, Retries) ->
    [begin
     ...
     || Miner <- Miners],
    ok = router_ct_utils:wait_for_app_stop(Miners, miner, Retries),
    ok.
...
start_miners(Miners, Retries) ->
    [begin
     ...
     || Miner <- Miners],
    ok = router_ct_utils:wait_for_registration(Miners, blockchain_worker, Retries),
    ok.
...
election_check(NotSeen0, Miners, AddrList, Owner) ->
    timer:sleep(500),
    Members = router_ct_utils:consensus_members(Miners),
    MinerNames = lists:map(fun(Member)-> router_ct_utils:addr2node(Member, AddrList) end, Members),
...
confirm_balance(Miners, Addr, Bal) ->
    ?assertAsync(begin
                     Result = lists:all(
                                fun(Miner) ->
                                        Bal == router_ct_utils:get_balance(Miner, Addr)
                                end, Miners)
                 end,
...
confirm_balance_both_sides(Miners, PayerAddr, PayeeAddr, PayerBal, PayeeBal) ->
    ?assertAsync(begin
                     Result = lists:all(
                                fun(Miner) ->
                                        PayerBal == router_ct_utils:get_balance(Miner, PayerAddr) andalso
                                            PayeeBal == router_ct_utils:get_balance(Miner, PayeeAddr)
                                end, Miners)
                 end,
...
inital_dkg(Miners, Txns, Addresses, NumConsensusMembers, Curve, Timeout)->
    DKGResults = router_ct_utils:pmap(
                   fun(Miner) ->
...
partition_miners(Members, AddrList) ->
    ...
    lists:partition(fun(Miner) ->
                            Addr = router_ct_utils:node2addr(Miner, AddrList),
...
load_genesis_block(GenesisBlock, Miners, Config) ->
    ...
    lists:foreach(
      fun(Miner) ->
              ...
              true = router_ct_utils:wait_until(
                       fun() ->
