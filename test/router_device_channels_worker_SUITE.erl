...
-export([refresh_channels_test/1, crashing_channel_test/1, late_packet_test/1]).
...
%%--------------------------------------------------------------------
all() ->
    [refresh_channels_test, crashing_channel_test, late_packet_test].
...
refresh_channels_test(Config) ->
    ...
    ?assertEqual(
        #{
            <<"HTTP_1">> =>
                convert_channel(State1#state.device, DeviceChannelsWorkerPid, HTTPChannel1),
            <<"HTTP_2">> =>
                convert_channel(State1#state.device, DeviceChannelsWorkerPid, HTTPChannel2)
        },
    ...
    ?assertEqual(
        #{
            <<"HTTP_1">> =>
                convert_channel(State2#state.device, DeviceChannelsWorkerPid, HTTPChannel1),
    ...
    ?assertEqual(
        #{
            <<"HTTP_2">> =>
                convert_channel(State3#state.device, DeviceChannelsWorkerPid, HTTPChannel2)
        },
...
crashing_channel_test(Config) ->
    ...
    meck:expect(router_http_channel, handle_info, fun (Msg, _State) ->
        erlang:throw(Msg)
    end),
    ...
    ?assertEqual(
        #{
            <<"HTTP_1">> =>
                convert_channel(State0#state.device, DeviceChannelsWorkerPid, HTTPChannel1)
        },
        State0#state.channels
    ),
    ...
    ?assertEqual(
        #{
            <<"HTTP_1">> =>
                convert_channel(State3#state.device, DeviceChannelsWorkerPid, HTTPChannel1)
        },
        State1#state.channels
    ),
...
late_packet_test(Config) ->
    ...
    {ok, HotspotName1} =
        erl_angry_purple_tiger:animal_name(libp2p_crypto:bin_to_b58(PubKeyBin1)),
    ...
    {ok, HotspotName2} =
        erl_angry_purple_tiger:animal_name(libp2p_crypto:bin_to_b58(PubKeyBin2)),
    Stream !
        {send,
            test_utils:frame_packet(
                ?UNCONFIRMED_UP,
...
convert_channel(Device, Pid, #{<<"type">> := <<"aws">>} = JSONChannel) ->
    ...
    Args = #{
        ...
        aws_region =>
            binary_to_list(kvc:path([<<"credentials">>, <<"aws_region">>], JSONChannel)),
        topic => kvc:path([<<"credentials">>, <<"topic">>], JSONChannel)
    },
    DeviceID = router_device:id(Device),
    Decoder = convert_decoder(JSONChannel),
