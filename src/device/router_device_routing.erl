    ...
    case find_device(PubKeyBin, DevAddr, MIC, Payload, Chain) of
        ...
        {Device, NwkSKey} ->
            case router_device:preferred_hotspots(Device) of
                ...
                Unexpected ->
                    lager:error(
                        "Ignoring invalid value ~p for preferred_hotspots in device ID ~p.", [
                            Unexpected, router_device:id(Device)
                        ]
                    ),
                    send_to_device_worker_(
                        Packet, PacketTime, HoldTime, Pid, PubKeyBin, Region, Device, NwkSKey
...
preferred_hotspot_test_init() ->
    ...
    meck:new(router_console_api, [passthrough]),
    meck:expect(router_console_api, get_devices_by_deveui_appeui, fun(_, _) -> [{key, Device}] end),
...
preferred_hotspot_test_cleanup(ValidateFun) ->
    ValidateFun(),
...
join_from_strange_hotspot_test() ->
    {ok, _} = preferred_hotspot_test_init(),

    JoinPacket = blockchain_helium_packet_v1:new({eui, 16#c0ffee, 16#7ea}, <<"payload">>),
    JoinOffer = blockchain_state_channel_offer_v1:from_packet(JoinPacket, <<"stranger">>, 'REGION'),

    preferred_hotspot_test_cleanup(fun() ->
        ?assertEqual({error, not_preferred_hotspot}, handle_offer(JoinOffer, self())),
        ok
    end),
    ok.

join_from_preferred_hotspot_test() ->
    {ok, _} = preferred_hotspot_test_init(),

    meck:new(router_device_multibuy),
    meck:expect(router_device_multibuy, maybe_buy, fun(_, _) ->
        throw("Multibuy isn't supposed to happen here!")
    end),

    JoinPacket = blockchain_helium_packet_v1:new({eui, 16#c0ffee, 16#7ea}, <<"payload">>),
    JoinOffer = blockchain_state_channel_offer_v1:from_packet(
        JoinPacket, <<"preferred1">>, 'REGION'
    ),
...
join_with_no_preferred_hotspot_test() ->
    {ok, {_DeviceID, Device}} = preferred_hotspot_test_init(),

    Device1 = router_device:update([{preferred_hotspots, []}], Device),

    % We have to change our expectation for this case.  It will be
    % unloaded automatically in preferred_hotspot_test_cleanup().
    meck:unload(router_console_api),
    meck:new(router_console_api),
    meck:expect(router_console_api, get_devices_by_deveui_appeui, fun(_, _) -> [{key, Device1}] end),

    % Multibuy should be used in this case.
    meck:new(router_device_multibuy, [passthrough]),
    meck:expect(router_device_multibuy, maybe_buy, fun(_, _) -> ok end),
    ...
    preferred_hotspot_test_cleanup(fun() ->
        ?assertEqual(meck:validate(router_device_multibuy), true),
        meck:unload(router_device_multibuy),
        ok
    end),
...
packet_from_strange_hotspot_test() ->
    {ok, {_DeviceID, Device}} = preferred_hotspot_test_init(),

    meck:new(router_device_cache, [passthrough]),
    meck:expect(router_device_cache, get_by_devaddr, fun(_) -> [Device] end),

    meck:new(router_device_devaddr, [passthrough]),
    meck:expect(router_device_devaddr, sort_devices, fun(_, _, _) -> [Device] end),

    meck:new(router_device_worker, [passthrough]),
    meck:expect(router_device_worker, accept_uplink, fun(_, _, _, _, _Hotspot) -> true end),

    meck:expect(router_device_worker, handle_frame, fun(_, _, _, _, _, _H, _, _) -> ok end),
    ...
    ?assertEqual(
        {error, not_preferred_hotspot},
        handle_packet(SCPacket, erlang:system_time(millisecond), self())
    ),

    ?assert(meck:validate(router_device_devaddr)),
    meck:unload(router_device_devaddr),

    ?assert(meck:validate(router_device_cache)),
    meck:unload(router_device_cache),

    ?assert(meck:validate(router_device_worker)),
    meck:unload(router_device_worker),

    preferred_hotspot_test_cleanup(fun() -> ok end),

    ok.

packet_with_no_preferred_hotspot_test() ->
    {ok, {_DeviceID, Device}} = preferred_hotspot_test_init(),

    Device1 = router_device:update([{preferred_hotspots, []}], Device),

    meck:new(router_device_cache, [passthrough]),
    meck:expect(router_device_cache, get_by_devaddr, fun(_) -> [Device1] end),

    meck:new(router_device_devaddr, [passthrough]),
    meck:expect(router_device_devaddr, sort_devices, fun(_, _, _) -> [Device1] end),
    ...
    ?assertEqual(ok, handle_packet(SCPacket, erlang:system_time(millisecond), self())),
...
packet_from_preferred_hotspot_test() ->
    {ok, {_DeviceID, Device}} = preferred_hotspot_test_init(),
    ...
    meck:new(router_device_worker, [passthrough]),
    meck:expect(router_device_worker, accept_uplink, fun(_, _, _, _, <<"preferred1">>) -> true end),

    meck:expect(router_device_worker, handle_frame, fun(_, _, _, _, _, <<"preferred1">>, _, _) ->
        ok
    end),
    meck:expect(router_metrics, routing_packet_observe, fun(_, _, _, _) -> ok end),
