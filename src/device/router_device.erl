...
-export([
    ...
    offset/1,
    offset/2,
    channel_correction/1,
    channel_correction/2,
    queue/1,
    queue/2,
    keys/1,
    keys/2,
...
new(ID) ->
    #device_v2{id = ID, keys = libp2p_crypto:generate_keys(ecc_compact)}.
...
-spec get(rocksdb:db_handle(), rocksdb:cf_handle(), binary()) ->
    {ok, device()} | {error, any()}.
get(DB, CF, DeviceID) ->
...
-spec save(rocksdb:db_handle(), rocksdb:cf_handle(), device()) ->
    {ok, device()} | {error, any()}.
save(DB, CF, Device) ->
...
new_test() ->
    ...
    ?assertEqual(#device_v2{id = <<"id">>, keys = Keys}, new(<<"id">>)),
...
upgrade_test() ->
    ...
    ?assertEqual({ok, V2Device}, get(DB, CF, DeviceID)),
    ?assertEqual([V2Device], get(DB, CF)),
