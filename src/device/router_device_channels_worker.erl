...
-spec handle_data(
    ...
    {libp2p_crypto:pubkey_bin(), #packet_pb{}, #frame{}, integer()}
) ->
    ok.
...
handle_cast(
    {handle_data, Device, {PubKeyBin, Packet, _Frame, _Time} = Data},
    ...
    DataCache1 =
        case FCnt =< CurrFCnt of
            true ->
                lager:debug(
                    "we received a late packet ~p from ~p: ~p",
                    [{FCnt, CurrFCnt}, PubKeyBin, Packet]
                ),
                DataCache0;
            false ->
...
handle_cast({report_status, Ref, Report}, #state{channels_resp_cache = Cache0} = State) ->
    ...
    Cache1 =
        case maps:get(Ref, Cache0, undefined) of
            undefined ->
                lager:warning(
                    "received report_status ~p too late ignoring ~p",
                    [Ref, Report]
                ),
                Cache0;
            {Data, CachedReports} ->
...
-spec downlink_decode(binary() | map()) ->
    {ok, {boolean(), integer(), binary()}} | {error, any()}.
...
send_to_channel(CachedData, Device, EventMgrRef) ->
    FoldFun = fun ({PubKeyBin, Packet, _, Time}, Acc) ->
    ...
    [{_, _, #frame{data = Data, fport = Port, fcnt = FCnt, devaddr = DevAddr}, Time} | _] =
        CachedData,
    Map = #{
...
start_channel(EventMgrRef, Channel, Device, Backoffs) ->
    ...
    case router_channel:add(EventMgrRef, Channel, Device) of
        ...
        {E, Reason} when E == 'EXIT'; E == error ->
            ...
            lager:error(
                "failed to start channel ~p: ~p, retrying in ~pms",
                [{ChannelID, ChannelName}, {E, Reason}, Delay]
            ),
...
update_channel(EventMgrRef, Channel, Device, Backoffs) ->
    ...
    case router_channel:update(EventMgrRef, Channel, Device) of
        ...
        {E, Reason} when E == 'EXIT'; E == error ->
            lager:error(
                "failed to update channel ~p: ~p",
                [{ChannelID, ChannelName}, {E, Reason}]
            ),
            Desc = erlang:list_to_binary(io_lib:format("~p ~p", [E, Reason])),
            Report = #{
    ...
    case router_channel:decoder(Channel) of
        ...
        Decoder ->
            ...
            case router_decoder:add(Decoder) of
                ...
                {error, _Reason} ->
                    lager:error(
                        "failed to attached decoder ~p to ~p: ~p",
                        [DecoderID, ChannelID, _Reason]
                    )
            end
    end.
