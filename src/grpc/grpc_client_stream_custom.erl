...
init({Connection, Service, Rpc, Encoder, Options, HandlerMod}) ->
    try
        ...
        lager:debug("init stream for RPC ~p and type ~p", [Rpc, StreamType]),
        Stream = new_stream(Connection, Service, Rpc, Encoder, Options),
        lager:debug("init stream success with state ~p, handle_mod: ~p", [Stream, HandlerMod]),
        HandlerState = HandlerMod:init(),
        {ok, Stream#{
            handler_state => HandlerState, handler_callback => HandlerMod, type => StreamType
        }}
    catch
...
process_response(Pid, Timeout) ->
    case rcv(Pid, Timeout) of
        ...
        {headers, #{<<"grpc-status">> := GrpcStatus} = Headers} when
            GrpcStatus == <<"14">>
        ->
            {error, #{
                error_type => http,
