                ...
                lorawan ->
                    case handle_lorawan_frame(Payload, AName) of
                        error ->
                            {error, decoding};
                        {join, Reply} ->
                            #{tmst := TxTime, datr := TxDataRate, freq := TxFreq} = lorawan_mac_region_old:join1_window(<<"US902-928">>,
                                                                                                                        #{<<"tmst">> => Time, <<"freq">> => Freq,
                            ...
                            case Frame#frame.mtype == ?CONFIRMED_UP orelse length(Device#device.queue) > 0 of
                                true ->
                                    ...
                                    DeviceUpdates = [
                                                     ...
                                                     {fcntdown, (Device#device.fcntdown + 1)}
                                                    ],
...
handle_lorawan_frame(<<MType:3, _MHDRRFU:3, _Major:2, DevAddr0:4/binary, ADR:1, ADRACKReq:1, ACK:1, RFU:1, FOptsLen:4, FCnt:16/little-unsigned-integer, FOpts:FOptsLen/binary, PayloadAndMIC/binary>> = Pkt, AName) ->
    ...
    case get_device_by_mic(router_devices_server:get_all(), <<(b0(MType band 1, DevAddr, FCnt, byte_size(Msg)))/binary, Msg/binary>>, MIC)  of
        undefined ->
