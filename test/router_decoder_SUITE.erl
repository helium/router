...
-export([decode_test/1, timeout_test/1, too_many_test/1]).
...
decode_test(Config) ->
    ...
    test_utils:wait_channel_data(#{
        ...
        <<"decoded">> =>
            #{<<"status">> => <<"success">>, <<"payload">> => #{<<"vSys">> => -0.5}},
        <<"port">> => 1,
        <<"devaddr">> => '_',
        <<"hotspots">> => [#{
            <<"id">> => erlang:list_to_binary(libp2p_crypto:bin_to_b58(PubKeyBin)),
...
timeout_test(Config) ->
    ...
    [{DecoderID, {custom_decoder, DecoderID, _Hash, _Fun, DecoderPid, _LastUsed}}] =
        ets:lookup(router_decoder_custom_sup_ets, DecoderID),
