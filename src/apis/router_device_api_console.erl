...
convert_channel(Device, Pid, #{<<"type">> := <<"http">>}=JSONChannel) ->
    ...
    Args = #{url =>  kvc:path([<<"credentials">>, <<"endpoint">>], JSONChannel),
             ...
             method => list_to_existing_atom(router_utils:to_bin(kvc:path([<<"credentials">>, <<"method">>], JSONChannel)))},
    DeviceID = router_device:id(Device),
...
convert_channel(Device, Pid, #{<<"type">> := <<"aws">>}=JSONChannel) ->
    ...
    Name = kvc:path([<<"name">>], JSONChannel),
    Args = #{aws_access_key => router_utils:to_bin(kvc:path([<<"credentials">>, <<"aws_access_key">>], JSONChannel)),
             aws_secret_key => router_utils:to_bin(kvc:path([<<"credentials">>, <<"aws_secret_key">>], JSONChannel)),
             aws_region => router_utils:to_bin(kvc:path([<<"credentials">>, <<"aws_region">>], JSONChannel)),
             topic => kvc:path([<<"credentials">>, <<"topic">>], JSONChannel)},
    DeviceID = router_device:id(Device),
    Decoder = convert_decoder(JSONChannel),
    Channel = router_channel:new(ID, Handler, Name, Args, DeviceID, Pid, Decoder),
