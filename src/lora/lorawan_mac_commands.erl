...
handle_fopts({Network, Profile, Node}, Gateways, ADR, FOpts) ->
    ...
    case FOptsIn of
        [] ->
            ok;
        List1 ->
            lager:debug("~s -> ~w", [lorawan_utils:binary_to_hex(Node#node.devaddr), List1])
    end,
    % process incoming responses
    {atomic, {MacConfirm, Node2}} = mnesia:transaction(
        fun () ->
...
handle_fopts0({Network, Profile, Node0}, Gateways, FOptsIn) ->
    {MacConfirm, Node1} = handle_rxwin(
        ...
        handle_adr(
            FOptsIn,
            handle_dcycle(FOptsIn, Profile, handle_status(FOptsIn, Network, Node0))
        )
    ),
    ...
    {_, RxQ} = hd(Gateways),
    {LastQs, AverageQs} =
        append_qs({RxQ#rxq.rssi, RxQ#rxq.lsnr}, Node1#node.last_qs, FramesRequired),
    Node2 =
        auto_adr(Network, Profile, Node1#node{last_qs = LastQs, average_qs = AverageQs}),
...
build_fopts({Network, Profile, Node}, FOptsOut0) ->
    FOptsOut = send_adr(
        ...
        set_dcycle(
            Profile,
            Node,
            set_rxwin(Profile, Node, request_status(Profile, Node, FOptsOut0))
        )
    ),
    case FOptsOut of
        [] ->
            ok;
        List2 ->
            lager:debug("~s <- ~w", [lorawan_utils:binary_to_hex(Node#node.devaddr), List2])
    end,
...
parse_fopts(
    <<16#05, _RFU:5, RX1DROffsetACK:1, RX2DataRateACK:1, ChannelACK:1, Rest/binary>>
) ->
...
parse_fdownopts(
    <<16#03, DataRate:4, TXPower:4, ChMask:16/little-unsigned-integer,
        0:1, ChMaskCntl:3, NbRep:4, Rest/binary>>
) ->
...
encode_fopts([{link_adr_req, DataRate, TXPower, ChMask, ChMaskCntl, NbRep} | Rest]) ->
    <<16#03, DataRate:4, TXPower:4, ChMask:16/little-unsigned-integer, 0:1,
        ChMaskCntl:3, NbRep:4, (encode_fopts(Rest))/binary>>;
...
encode_fopts([{rx_param_setup_req, RX1DROffset, RX2DataRate, Frequency} | Rest]) ->
    <<16#05, 0:1, RX1DROffset:3, RX2DataRate:4,
        Frequency:24/little-unsigned-integer, (encode_fopts(Rest))/binary>>;
encode_fopts([dev_status_req | Rest]) ->
    <<16#06, (encode_fopts(Rest))/binary>>;
encode_fopts([{new_channel_req, ChIndex, Freq, MaxDR, MinDR} | Rest]) ->
    <<16#07, ChIndex, Freq:24/little-unsigned-integer, MaxDR:4,
        MinDR:4, (encode_fopts(Rest))/binary>>;
...
encode_fopts([{device_time_ans, MsSinceEpoch} | Rest]) ->
    ...
    <<16#0D, (MsSinceEpoch div 1000):32/little-unsigned-integer,
        Ms, (encode_fopts(Rest))/binary>>;
encode_fopts([]) ->
    <<>>.

encode_fupopts([{link_adr_ans, PowerACK, DataRateACK, ChannelMaskACK} | Rest]) ->
    <<16#03, 0:5, PowerACK:1, DataRateACK:1, ChannelMaskACK:1,
...
handle_status(FOptsIn, #network{region = Region}, Node) ->
    case find_status(FOptsIn) of
        {Battery, Margin} ->
            ...
            lager:debug(
                "DevStatus: battery ~B, margin: ~B (max ~.1f)",
                [Battery, Margin, MaxSNR]
            ),
            Node#node{
...
command_test_() ->
    [
        ...
        ?_assertEqual(
            ...
            send_device_time([
                {<<>>, #rxq{time = {{2016, 2, 12}, {14, 24, 31}}, tmms = undefined}}
            ])
        )
    ].
