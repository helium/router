...
-spec decode(router_decoder:decoder(), binary(), integer()) ->
    {ok, binary()} | {error, any()}.
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?DIGITAL_IN:8/integer,
        Value:8/unsigned-integer, Rest/binary>>,
    Acc
) ->
    %% TODO this should be 0 or 1
    decode_lpp(Rest, [
        #{channel => Channel, type => ?DIGITAL_IN, value => Value, name => digital_in} | Acc
    ]);
decode_lpp(
    <<Channel:8/unsigned-integer, ?DIGITAL_OUT:8/integer,
        Value:8/unsigned-integer, Rest/binary>>,
    Acc
) ->
    %% TODO this should be 0 or 1
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?ANALOG_IN:8/integer,
        Value:16/signed-integer, Rest/binary>>,
    Acc
) ->
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?ANALOG_OUT:8/integer,
        Value:16/big-signed-integer, Rest/binary>>,
    Acc
) ->
    %% TODO is the value MSB or LSB
    decode_lpp(Rest, [
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?LUMINANCE:8/integer,
        Value:16/integer-unsigned-big, Rest/binary>>,
    Acc
) ->
    %% TODO is the value MSB or LSB
    decode_lpp(Rest, [
...
decode_lpp(
    ...
    decode_lpp(Rest, [
        #{channel => Channel, type => ?PRESENCE, value => Value, name => presence} | Acc
    ]);
decode_lpp(
    <<Channel:8/unsigned-integer, ?TEMPERATURE:8/integer,
        Value:16/integer-signed-big, Rest/binary>>,
    Acc
) ->
    %% TODO value is 0 or 1
    decode_lpp(Rest, [
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?HUMIDITY:8/integer,
        Value:8/integer-unsigned-big, Rest/binary>>,
    Acc
) ->
    decode_lpp(Rest, [
        #{
...
decode_lpp(
    <<Channel:8/unsigned-integer, ?BAROMETER:8/integer,
        Value:16/integer-unsigned-big, Rest/binary>>,
    Acc
) ->
    decode_lpp(Rest, [
...
decode_lpp(
    ...
    decode_lpp(Rest, [
        #{
            ...
            type => ?GPS,
            value =>
                #{latitude => Lat / 10000, longitude => Lon / 10000, altitude => Alt / 100},
            name => gps
        }
        | Acc
    ]);
