...
init_per_testcase(TestCase, Config) ->
    ...
    ok = application:set_env(router, router_device_api_console, [
    ...
    case os:getenv("CT_LAGER", "NONE") of
        "DEBUG" ->
            ...
            ok = application:set_env(lager, handlers, [
                {lager_console_backend, [{level, debug}, {formatter_config, FormatStr}]}
            ]);
        _ ->
...
start_swarm(BaseDir, Name, Port) ->
    ...
    ct:pal(
        "created swarm ~p @ ~p p2p address=~p",
        [Name, Swarm, libp2p_swarm:p2p_address(Swarm)]
    ),
    Swarm.
        ...
        receive
            {client_data, PubKeyBin, Data} ->
                ...
                of
                    ...
                    _E:_R ->
                        ct:fail(
                            "wait_state_channel_message failed to decode ~p ~p",
                            [Data, {_E, _R}]
                        )
                end
        ...
        receive
            {client_data, undefined, Data} ->
                ...
                of
                    #blockchain_state_channel_message_v1_pb{msg = {response, Resp}} ->
                        case Resp of
                            ...
                            #blockchain_state_channel_response_v1_pb{
                                ...
                                Frame =
                                    deframe_packet(Packet, router_device:app_s_key(Device)),
                    ...
                    _Else ->
                        ct:fail(
                            "wait_state_channel_message wrong message ~p for ~p",
                            [_Else, Msg]
                        )
                catch
                    _E:_R ->
                        ct:fail(
                            "wait_state_channel_message failed to decode ~p ~p for ~p",
                            [Data, {_E, _R}, Msg]
                        )
                end
...
join_payload(AppKey, DevNonce) ->
    ...
    Payload0 =
        <<MType:3, MHDRRFU:3, Major:2, AppEUI:8/binary,
            DevEUI:8/binary, DevNonce:2/binary>>,
...
frame_payload(MType, DevAddr, NwkSessionKey, AppSessionKey, FCnt, Options) ->
    ...
    Payload0 =
        <<MType:3, MHDRRFU:3, Major:2, DevAddr:4/binary, ADR:1, ADRACKReq:1,
            ACK:1, RFU:1, FOptsLen:4, FCnt:16/little-unsigned-integer,
                ...
                fun
                    ...
                    (K, V, true) when is_function(V) ->
                        case V(maps:get(K, Got, undefined)) of
                            ...
                            false ->
                                {false,
                                    {value_predicate_failed, K,
                                        maps:get(K, Got, undefined)}}
                        end;
                    (K, '_', true) ->
                    ...
                    (K, V, true) ->
                        case maps:get(K, Got, undefined) of
                            V ->
                                true;
                            _ ->
                                {false, {value_mismatch, K, V, maps:get(K, Got, undefined)}}
                        end
                end,
                true,
                Expected
...
deframe_packet(Packet, SessionKey) ->
    <<MType:3, _MHDRRFU:3, _Major:2, DevAddr:4/binary, ADR:1, RFU:1,
        ACK:1, FPending:1, FOptsLen:4, FCnt:16/little-unsigned-integer,
        FOpts:FOptsLen/binary, PayloadAndMIC/binary>> = Packet#packet_pb.payload,
...
deframe_join_packet(
    ...
    <<AppNonce:3/binary, NetID:3/binary, DevAddr:4/binary,
        DLSettings:8/integer-unsigned, RxDelay:8/integer-unsigned, MIC:4/binary>> =
