...
handle_cast({join, Packet0, PubkeyBin, AppKey, DeviceName, Pid}, #state{device=Device0,
                                                                        join_cache=Cache0}=State0) ->
    case handle_join(Packet0, PubkeyBin, AppKey, DeviceName, Device0) of
        {error, _Reason} ->
            {noreply, State0};
    ...
    case maps:is_key(ChannelID, Channels) of
        ...
        false ->
            case add_channel(EventMgrRef, Channel, Device) of
                ...
                {error, _Reason} ->
                    lager:error("failed to restart channel ~p: ~p, stopping here", [ChannelID, _Reason]),
                    {noreply, State#state{channels_backoffs=maps:remove(ChannelID, Backoffs)}}
            end
    end;
handle_info({started, ChannelID}, #state{channels_backoffs=Backoffs}=State) ->
    Init = {backoff:type(backoff:init(?BACKOFF_MIN, ?BACKOFF_MAX), normal), erlang:make_ref()},
    {Backoff0, TimerRef0} = maps:get(ChannelID, Backoffs, Init),
    _ = erlang:cancel_timer(TimerRef0),
    {_, Backoff1} = backoff:succeed(Backoff0),
    {noreply, State#state{channels_backoffs=maps:put(ChannelID, {Backoff1, erlang:make_ref()}, Backoffs)}};
handle_info({gen_event_EXIT, {_Handler, ChannelID}, Reason}, #state{device=Device, channels=Channels,
                                                                    channels_backoffs=Backoffs}=State) ->
    case Reason of
        normal ->
            {noreply, State#state{channels=maps:remove(ChannelID, Channels),
                                  channels_backoffs=maps:remove(ChannelID, Backoffs)}};
        shutdown ->
            {noreply, State#state{channels=maps:remove(ChannelID, Channels),
                                  channels_backoffs=maps:remove(ChannelID, Backoffs)}};
        ...
        Error ->
            ...
            router_device_api:report_channel_status(Device, #{channel_id => ChannelID, channel_name => Name, status => failure,
            ...
            TimerRef1 = erlang:send_after(Delay, self(), {restart_channel, Channel}),
            {noreply, State#state{channels=maps:remove(ChannelID, Channels),
                                  channels_backoffs=maps:put(ChannelID, {Backoff1, TimerRef1}, Backoffs)}}
    end;
        ...
        {ok, Device, AppKey} ->
            ...
            case maybe_start_worker(DeviceID) of
                {error, _Reason}=Error ->
                    Error;
                {ok, WorkerPid} ->
                    DeviceName = router_device:name(Device),
                    gen_server:cast(WorkerPid, {join, Packet, PubkeyBin, AppKey, DeviceName, Pid})
            end;
        {error, api_not_found} ->
...
handle_join(#packet_pb{payload= <<MType:3, _MHDRRFU:3, _Major:2, _AppEUI0:8/binary,
    ...
    handle_join(Packet, PubkeyBin, AppKey, DeviceName, Device, router_device:join_nonce(Device));
handle_join(_Packet, _PubkeyBin, _AppKey, _DeviceName, _Device) ->
    {error, not_join_req}.
...
handle_join(#packet_pb{payload= <<_MType:3, _MHDRRFU:3, _Major:2, AppEUI0:8/binary,
    ...
    case throttle:check(join_dedup, {AppEUI, DevEUI, Nonce}) of
        {ok, _, _} ->
            lager:debug("Device ~s ~p ~p tried to join with stale nonce ~p via ~s", [DeviceName, OUI, DID, Nonce, AName]),
...
handle_join(#packet_pb{oui=OUI, type=Type, timestamp=Time, frequency=Freq, datarate=DataRate,
            ...
            PubkeyBin, AppKey, DeviceName, Device0, _OldNonce) ->
    {ok, AName} = erl_angry_purple_tiger:animal_name(libp2p_crypto:bin_to_b58(PubkeyBin)),
    {AppEUI, DevEUI} = {lorawan_utils:reverse(AppEUI0), lorawan_utils:reverse(DevEUI0)},
    NetID = <<"He2">>,
