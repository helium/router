...
-export([
    ...
    preferred_hotspots/1, preferred_hotspots/2,
    update/2,
    serialize/1,
    deserialize/1
]).
...
devaddr(Device) ->
    Device#device_v7.devaddr.

-spec devaddr(binary(), device()) -> device().
devaddr(Devaddr, Device) ->
    Device#device_v7{devaddr = Devaddr}.
...
-spec preferred_hotspots(device()) -> [binary()] | [].
preferred_hotspots(Device) ->
    Device#device_v7.preferred_hotspots.

-spec preferred_hotspots(list(binary()), device()) -> device().
preferred_hotspots(Hotspots, Device) ->
    Device#device_v7{preferred_hotspots = Hotspots}.
...
update([{preferred_hotspots, Value} | T], Device) ->
    update(T, ?MODULE:preferred_hotspots(Value, Device)).
...
can_queue_payload(_Payload, #device_v7{region = undefined}) ->
...
preferred_hotspots_test() ->
    Device = new(<<"id">>),
    ?assertMatch([], preferred_hotspots(Device)).
...
update_test() ->
    ...
    Updates = [
        ...
        {metadata, #{a => b}},
        {is_active, false},
        {preferred_hotspots, [<<"preferred_1">>]}
    ],
