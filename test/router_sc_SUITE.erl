...
init_per_testcase(TestCase, Config0) ->
    ...
    AddressesWithLocations =
        lists:zip(Addresses, lists:sublist(Locations, length(Addresses))),
    AddGwTxns =
        [
    ...
    ?assert(
        lists:member({save_block, 2}, proplists:get_value(exports, RouterChainModInfo))
    ),
...
maintain_channels_test(Config) ->
    ...
    %% setup
    %% oui txn
    {ok, RouterPubkey, RouterSigFun, _ECDHFun} =
    ...
    {Filter, _} =
        xor16:to_bin(xor16:new([<<DevEUI/binary, AppEUI/binary>>], fun xxhash:hash64/1)),
    ...
    SignedOUITxn =
        ct_rpc:call(RouterNode, blockchain_txn_oui_v1, sign, [OUITxn, RouterSigFun]),
    ...
    {ok, 1} =
        ct_rpc:call(RouterNode, blockchain_ledger_v1, get_oui_counter, [RouterLedger]),
    ...
    true = miner_test:wait_until(
        fun () ->
            RouterChain = ct_rpc:call(RouterNode, blockchain_worker, blockchain, []),
            {ok, RouterChainHeight} =
                ct_rpc:call(RouterNode, blockchain, height, [RouterChain]),
    ...
    true = miner_test:wait_until(
        fun () ->
            ...
            RouterLedger = ct_rpc:call(RouterNode, blockchain, ledger, [RouterChain]),
            {ok, LedgerSCs} =
                ct_rpc:call(RouterNode, blockchain_ledger_v1, find_scs_by_owner, [
                    RouterPubkeyBin,
                    RouterLedger
                ]),
            {_, S} = hd(
                lists:sort(
                    fun ({_, S1}, {_, S2}) ->
                        blockchain_ledger_state_channel_v1:nonce(S1) >=
                            blockchain_ledger_state_channel_v1:nonce(S2)
                    end,
...
handle_packets_test(Config) ->
    ...
    ClientPubkeyBins =
        [ct_rpc:call(Miner, blockchain_swarm, pubkey_bin, []) || Miner <- Miners],
    ...
    {DevAddr, NetKey, AppKey} =
        receive
            {fake_radio_backplane, ReplyPacket} ->
                ...
                {_NetID, DA, _DLSettings, _RxDelay, NK,
                    AK} = test_utils:deframe_join_packet(
...
no_dc_entry_test(Config) ->
    ...
    OUITxn = ct_rpc:call(
        ...
        blockchain_txn_oui_v1,
        new,
        [OUI, RouterPubkeyBin, [RouterPubkeyBin], Filter, 8, PayerPubkeyBin, 1, 0]
    ),
    ct:pal("OUITxn: ~p", [OUITxn]),
    SignedOUITxn0 =
        ct_rpc:call(RouterNode, blockchain_txn_oui_v1, sign, [OUITxn, RouterSigFun]),
    ...
    SignedOUITxn = ct_rpc:call(PayerNode, blockchain_txn_oui_v1, sign_payer, [
        SignedOUITxn0,
        PayerSigFun
    ]),
    ct:pal("SignedOUITxn: ~p", [SignedOUITxn]),
