...
init(client, _Conn, [Pid, Pubkeybin] = _Args) ->
    ...
    Port = erlang:open_port(
        {spawn_executable, "../../../../priv/LoRaMac-classA"},
        [{line, 65535}, stream, binary, exit_status]
    ),
...
handle_info(
    client,
    {Port,
        {data, {eol, <<"{\"rxpk\"", _/binary>> = JSONBin}}},
    ...
    State1 =
        case LogMsg of
            ...
            <<"###### ===== JOINING ==== ######">> ->
                ...
                State;
            <<"CHANNEL MASK: ", A:4/binary,
                " ", B:4/binary, " ",
                C:4/binary, " ", D:4/binary,
                " ", E:4/binary, _/binary>> ->
...
handle_info(client, get_channel_mask, State) ->
    State#state.pid !
        {channel_mask, State#state.channel_mask},
    {noreply, State};
