...
init(client, _Conn, [Pid, Pubkeybin]=_Args) ->
    ...
    Port = erlang:open_port({spawn_executable, "../../../../priv/LoRaMac-classA"}, [{line, 65535}, stream, binary, exit_status]),
    ...
    try blockchain_state_channel_v1_pb:decode_msg(Data, blockchain_state_channel_message_v1_pb) of
        #blockchain_state_channel_message_v1_pb{msg={response, Resp}} ->
            ...
            case Packet of
                ...
                _ ->
                    TxPk = jsx:encode(#{txpk => #{data => base64:encode(Packet#packet_pb.payload), size => byte_size(Packet#packet_pb.payload), freq => Packet#packet_pb.frequency, datr => Packet#packet_pb.datarate}}),
...
handle_info(client, {send, Data}, State) ->
    {noreply, State, Data};
handle_info(client, {Port, {data,{eol,<<"{\"rxpk\"", _/binary>> = JSONBin}}}, State = #state{port=Port}) ->
...
handle_info(client, {Port, {data,{eol,LogMsg}}}, State = #state{port=Port}) ->
    State1 = case LogMsg of
                 ...
                 <<"###### ===== JOINING ==== ######">> ->
                     ...
                     State;
                 <<"CHANNEL MASK: ", A:4/binary, " ", B:4/binary, " ", C:4/binary, " ", D:4/binary, " ", E:4/binary, _/binary>> ->
...
parse_channel_mask(B) ->
    {_, R} = lists:foldl(fun(Byte, {Offset, Channels}) ->
                                 NewChannels = [Offset * (Byte band 2#1), (Offset + 1) * ((Byte band 2#10) bsr 1),
                                                (Offset + 2) * ((Byte band 2#100) bsr 2), (Offset + 3) * ((Byte band 2#1000) bsr 3),
                                                (Offset + 4) * ((Byte band 2#10000) bsr 4), (Offset + 5) * ((Byte band 2#100000) bsr 5),
                                                (Offset + 6) * ((Byte band 2#1000000) bsr 6), (Offset + 7) * ((Byte band 2#10000000) bsr 7)],
                                 {Offset + 8,
                                  Channels ++ NewChannels}
                         end, {1, []},
                         binary_to_list( << <<Y:8/integer, X:8/integer>> || <<X:8/integer, Y:8/integer>> <= lorawan_utils:hex_to_binary(B) >>)),
    [ E - 1 || E <- R, E /= 0 ].
