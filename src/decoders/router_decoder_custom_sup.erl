...
%% API
-export([start_link/0, add/1, delete/1, decode/3]).
...
-define(FLAGS, #{strategy => simple_one_for_one, intensity => 3, period => 60}).
...
decode(Decoder, Payload, Port) ->
    ...
    case lookup(ID) of
        ...
        {ok, #custom_decoder{pid = Pid} = CustomDecoder} ->
            ok = insert(CustomDecoder#custom_decoder{
...
get_oldest_decoder() ->
    ...
    case ets:info(?ETS, size) + 1 > Max of
        false ->
            undefined;
        true ->
            SortFun = fun ([{_, #custom_decoder{last_used = T1}}], [
