...
handle_event(
    ...
    Body = router_channel:encode_data(
        Channel,
        maps:merge(Data, #{downlink_url => DownlinkURL})
    ),
    Res = make_http_req(Method, URL, Headers, Body),
    lager:debug("published: ~p result: ~p", [Body, Res]),
    Debug = #{req => #{method => Method, url => URL, headers => Headers, body => Body}},
...
handle_call({update, Channel, _Device}, State) ->
    ...
    {ok, ok,
        State#state{channel = Channel, url = URL, headers = Headers1, method = Method}};
        ...
        case Res of
            {ok, StatusCode, ResponseHeaders, <<>>}
                ...
                maps:merge(Result0, #{
                    debug => maps:merge(Debug, #{
                        res =>
                            #{code => StatusCode, headers => ResponseHeaders, body => <<>>}
                    }),
                    status => success,
            ...
            {ok, StatusCode, ResponseHeaders, ResponseBody} ->
                maps:merge(Result0, #{
                    ...
                    description =>
                        <<"ResponseCode: ",
                            (list_to_binary(integer_to_list(StatusCode)))/binary,
