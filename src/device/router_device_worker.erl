...
%% ------------------------------------------------------------------
-export([start_link/1, handle_packet/2, queue_message/2, device_update/1]).
...
handle_packet(SCPacket, Pid) when is_pid(Pid) ->
    ...
    PubkeyBin =
        blockchain_state_channel_packet_v1:hotspot(
            SCPacket
        ),
    Region =
        blockchain_state_channel_packet_v1:region(
            SCPacket
        ),
    case handle_packet(
             Packet,
        ...
        {error, _Reason} = E ->
            lager:info(
                "failed to handle sc packet ~p : ~p",
                [Packet, _Reason]
            ),
            E;
        ok ->
...
init(Args) ->
    ...
    OUI =
        case application:get_env(router, oui, undefined) of
            undefined -> undefined;
            OUI0 when is_list(OUI0) -> list_to_integer(OUI0);
            OUI0 -> OUI0
        end,
...
handle_cast(
    {queue_message, {_Type, _Port, _Payload} = Msg},
    #state{db = DB, cf = CF, device = Device0, channels_worker = ChannelsWorker} = State
) ->
...
handle_cast(
    ...
    {noreply, State0};
handle_cast(
    {join, Packet0, PubKeyBin, Region, APIDevice, AppKey, Pid},
    #state{
        db = DB,
        cf = CF,
        device = Device0,
...
handle_cast(
    {frame, Packet0, PubKeyBin, Region, Pid},
    #state{
        device = Device0,
        frame_cache = Cache0,
...
handle_info(
    ...
    #state{db = DB, cf = CF, channels_worker = ChannelsWorker, join_cache = Cache0} = State
) ->
    #join_cache{
        reply = Reply,
        ...
        pubkey_bin = PubKeyBin,
        region = Region
    } = maps:get(JoinNonce, Cache0),

    #txq{time = TxTime, datr = TxDataRate, freq = TxFreq} =
        lorawan_mac_region:join1_window(Region, 0, packet_to_rxq(PacketRcvd)),

    Packet = blockchain_helium_packet_v1:new_downlink(
        Reply,
        TxTime,
        27,
        TxFreq,
        binary_to_list(TxDataRate)
              ...
              Pid,
              blockchain_state_channel_response_v1:new(true, Packet)
          ),
    ...
    Desc =
        <<"Join attempt from AppEUI: ", (lorawan_utils:binary_to_hex(AppEUI))/binary,
...
handle_info(
    ...
    lager:debug(
        "frame timeout for ~p / device ~p",
        [FCnt, lager:pr(Device, router_device)]
    ),
...
%% @end
%%%-------------------------------------------------------------------
-spec handle_packet(blockchain_helium_packet_v1:packet(), string(), atom(), pid()) ->
    ok | {error, any()}.
handle_packet(
    #packet_pb{
        payload =
            <<MType:3, _MHDRRFU:3, _Major:2,
                AppEUI0:8/binary, DevEUI0:8/binary,
                _DevNonce:2/binary, MIC:4/binary>> =
                Payload
    } = Packet,
    PubKeyBin,
    Region,
    ...
    {AppEUI, DevEUI} =
        {lorawan_utils:reverse(AppEUI0),
            lorawan_utils:reverse(DevEUI0)},
    {ok, AName} = erl_angry_purple_tiger:animal_name(
        libp2p_crypto:bin_to_b58(PubKeyBin)
    ),
    Msg = binary:part(
        Payload,
        {0, erlang:byte_size(Payload) - 4}
    ),
    case router_device_api:get_device(
             DevEUI,
             AppEUI,
             Msg,
             MIC
...
handle_packet(
    #packet_pb{
        payload =
            <<MType:3, _MHDRRFU:3, _Major:2, DevAddr:4/binary, _ADR:1, _ADRACKReq:1,
                _ACK:1, _RFU:1, FOptsLen:4, FCnt:16/little-unsigned-integer,
                _FOpts:FOptsLen/binary, PayloadAndMIC/binary>> = Payload
    } = Packet,
    PubKeyBin,
    Region,
    Pid
        ...
        <<AddrBase:25/integer-unsigned-little, DevAddrPrefix:7/integer>> ->
            ...
            try blockchain_ledger_v1:find_routing(OUI, blockchain:ledger(Chain)) of
                {ok, RoutingEntry} ->
                    ...
                    case lists:any(
                             fun (Subnet) ->
                                 <<Base:25/integer-unsigned-big,
                                     Mask:23/integer-unsigned-big>> = Subnet,
...
-spec handle_join(
    ...
    {ok, Reply :: binary(), Device :: router_device:device(),
        DevAddr :: binary()} | {error, any()}.
handle_join(
    #packet_pb{
        payload =
            <<MType:3, _MHDRRFU:3, _Major:2, _AppEUI0:8/binary,
                _DevEUI0:8/binary, _Nonce:2/binary, _MIC:4/binary>>
    } = Packet,
    PubKeyBin,
    Region,
    OUI,
    ...
    handle_join(
        ...
        AppKey,
        Device,
        router_device:join_nonce(Device)
    );
...
handle_join(
    #packet_pb{
        payload =
            <<_MType:3, _MHDRRFU:3, _Major:2, AppEUI0:8/binary,
    ...
    {AppEUI, DevEUI} =
        {lorawan_utils:reverse(AppEUI0),
            lorawan_utils:reverse(DevEUI0)},
    lager:warning(
        "~s ~s tried to join with stale nonce ~p via ~s",
        [
...
handle_join(
    ...
    NwkSKey = crypto:block_encrypt(
        ...
        lorawan_utils:padded(
            16,
            <<16#01, AppNonce/binary,
                NetID/binary, DevNonce/binary>>
        )
    ),
    AppSKey = crypto:block_encrypt(
        ...
        lorawan_utils:padded(
            16,
            <<16#02, AppNonce/binary,
                NetID/binary, DevNonce/binary>>
        )
    ),
    Chain = blockchain_worker:blockchain(),
    DevAddrPrefix =
        application:get_env(blockchain, devaddr_prefix, $H),
    DevAddr =
        try
            blockchain_ledger_v1:find_routing(OUI, blockchain:ledger(Chain))
        of
            {ok, RoutingEntry} ->
                Subnets = blockchain_ledger_routing_v1:subnets(RoutingEntry),
                <<Base:25/integer-unsigned-big,
                    _Mask:23/integer-unsigned-big>> = hd(Subnets),
    ...
    ReplyPayload =
        <<AppNonce/binary, NetID/binary,
            DevAddr/binary,
            DLSettings:8/integer-unsigned,
            RxDelay:8/integer-unsigned, CFList/binary>>,
    ReplyMIC =
        crypto:cmac(aes_cbc128, AppKey, <<ReplyHdr/binary, ReplyPayload/binary>>, 4),
    EncryptedReply =
...
validate_frame(Packet, PubKeyBin, Region, Device0) ->
    <<MType:3, _MHDRRFU:3, _Major:2, DevAddr:4/binary, ADR:1, ADRACKReq:1, ACK:1,
        RFU:1, FOptsLen:4, FCnt:16/little-unsigned-integer, FOpts:FOptsLen/binary,
    ...
    case FPort of
        ...
        0 when FOptsLen /= 0 ->
            ...
            ok =
                report_status(up, Desc, Device0, error, PubKeyBin, Region, Packet, 0, DevAddr),
            {error, double_fopts};
        _N ->
...
handle_frame(
    ...
    lager:info(
        "downlink with no queue, ACK ~p and channels corrected ~p",
        [ACK, ChannelCorrection orelse WereChannelsCorrected]
    ),
    ...
    case ACK of
        _ when ACK == 1 orelse FOpts1 /= [] ->
            ...
            case ChannelCorrection == false andalso
                     WereChannelsCorrected == true of
                true ->
                    {send,
                        router_device:channel_correction(
                            true,
                            Device1
                        ),
                        Packet1};
...
handle_frame(Packet0, PubKeyBin, Region, Device0, Frame, Count, [
    {ConfirmedDown, Port, ReplyPayload} | T
]) ->
    ACK =
        mtype_to_ack(Frame#frame.mtype),
    MType =
        ack_to_mtype(ConfirmedDown),
    WereChannelsCorrected =
        were_channels_corrected(
            Frame,
            Region
        ),
    ...
    case ConfirmedDown of
        true ->
            Device1 = router_device:channel_correction(
                ChannelsCorrected,
                Device0
            ),
            ok = report_frame_status(
...
channel_correction_and_fopts(
    ...
    DataRate =
        blockchain_helium_packet_v1:datarate(
            Packet
        ),
    ChannelCorrection =
        router_device:channel_correction(
    ...
    ChannelCorrectionNeeded =
        ChannelCorrection == false,
    FOpts1 =
    ...
    FOpts2 =
        case lists:member(link_check_req, Frame#frame.fopts) of
            true ->
                ...
                lager:info(
                    "respond to link_check_req with link_check_ans ~p ~p",
                    [Margin, Count]
                ),
...
-spec report_frame_status(
    ...
    ok.
report_frame_status(0, false, 0, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr,
    fport = FPort
}) ->
...
report_frame_status(1, _ConfirmedDown, undefined, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr,
    fport = FPort
}) ->
    FCnt = router_device:fcnt(Device),
...
report_frame_status(1, true, Port, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr
}) ->
    FCnt = router_device:fcnt(Device),
    Desc =
        <<"Sending ACK and confirmed data in response to fcnt ",
            (int_to_bin(FCnt))/binary>>,
    ok =
        report_status(ack, Desc, Device, success, PubKeyBin, Region, Packet, Port, DevAddr);
report_frame_status(1, false, Port, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr
}) ->
    FCnt = router_device:fcnt(Device),
    Desc =
        <<"Sending ACK and unconfirmed data in response to fcnt ",
            (int_to_bin(FCnt))/binary>>,
    ok =
        report_status(ack, Desc, Device, success, PubKeyBin, Region, Packet, Port, DevAddr);
report_frame_status(_, true, Port, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr
}) ->
    FCnt = router_device:fcnt(Device),
...
report_frame_status(_, false, Port, PubKeyBin, Region, Device, Packet, #frame{
    devaddr = DevAddr
}) ->
    FCnt = router_device:fcnt(Device),
...
frame_to_packet_payload(Frame, Device) ->
    ...
    PktHdr =
        <<(Frame#frame.mtype):3, 0:3, 0:2,
            (Frame#frame.devaddr)/binary, (Frame#frame.adr):1, 0:1,
    ...
    MIC = crypto:cmac(
        aes_cbc128,
        NwkSKey,
        <<(b0(1, Frame#frame.devaddr, Frame#frame.fcnt, byte_size(Msg)))/binary,
            Msg/binary>>,
        4
    ),
    <<Msg/binary, MIC/binary>>.

-spec get_device_by_mic(rocksdb:db_handle(), rocksdb:cf_handle(), binary(), binary()) ->
