...
device_worker_late_packet_double_charge_test(Config) ->
    %% FIXME: Multiple runs to make sure we trigger a late packet
    [
        ok = device_worker_late_packet_double_charge_testx(Config, 1),
        ok = device_worker_late_packet_double_charge_testx(Config, 2),
        ok = device_worker_late_packet_double_charge_testx(Config, 3),
        ok = device_worker_late_packet_double_charge_testx(Config, 4),
        ok = device_worker_late_packet_double_charge_testx(Config, 5),
        ok = device_worker_late_packet_double_charge_testx(Config, 6),
        ok = device_worker_late_packet_double_charge_testx(Config, 7)
    ].

device_worker_late_packet_double_charge_testx(Config, RunNum) ->
    #{stream := Stream, pubkey_bin := PubKeyBin1, hotspot_name := _HotspotName1} = join(Config),
    ...
    FirstReport = fun() ->
        {B, N} = router_console_dc_tracker:current_balance(?CONSOLE_ORG_ID),
        S = io_lib:format("~p -> [balance: ~p] [nonce: ~p]", [?CONSOLE_ORG_ID, B, N]),
        {ok, B, N, S}
    end,
    CompareReport = fun({ok, OB, ON, _}) ->
        {B, N} = router_console_dc_tracker:current_balance(?CONSOLE_ORG_ID),
        S = io_lib:format(
            "Old -> [old_balance: ~p] [old_nonce: ~p]\n"
            "New -> [new_balance: ~p] [new_nonce: ~p]\n"
            "Dif -> [dif_balance: ~p] [dif_nonce: ~p]",
            [OB, ON, B, N, OB - B, ON - N]
        ),
        {ok, B, N, S}
    end,

    OG_BN = {ok, _StartingBalance, _StartingNonce, Print1} = FirstReport(),

    %% XTODO: Set multi-buy to 1
    %% multi-buy is 1 by default
    ...
    %% TODO: Passing test should only have 1 hotspot listed
    test_utils:wait_channel_data(#{
        ...
        <<"port">> => 1,
        <<"devaddr">> => '_',
        <<"dc">> => #{<<"balance">> => _StartingBalance - 1, <<"nonce">> => _StartingNonce},
        <<"hotspots">> => [
            #{
                <<"id">> => erlang:list_to_binary(libp2p_crypto:bin_to_b58(PubKeyBin1)),
                <<"name">> => erlang:list_to_binary(_HotspotName1),
                <<"reported_at">> => fun erlang:is_integer/1,
                <<"status">> => <<"success">>,
                <<"rssi">> => 0.0,
                <<"snr">> => 0.0,
            ...
            %% NOTE: This hotspot shows up when late packets are mishandled
            %% #{
            %%     <<"id">> => erlang:list_to_binary(libp2p_crypto:bin_to_b58(PubKeyBin2)),
            %%     <<"name">> => erlang:list_to_binary(_HotspotName2),
            %%     <<"reported_at">> => fun erlang:is_integer/1,
            %%     <<"status">> => <<"success">>,
            %%     <<"rssi">> => 0.0,
            %%     <<"snr">> => 0.0,
    ...
    test_utils:wait_for_console_event(<<"down">>, #{
        ...
        %% FIXME: Why does uplink have a channel but not downlink
        <<"channels">> => []
    }),
    ...
    {ok, _EndingBalance, _EndingNonce, Print2} = CompareReport(OG_BN),

    ?assertEqual(_EndingBalance, _StartingBalance - 1),
    ?assertEqual(_EndingNonce, _StartingNonce),

    ct:print(lists:join("\n\n", [Print1, Print2])),

    %% REVIEW: Reading printout
    %% look for "not validating because: late_packet"
    %% Those are the runs that would have charged 2 DC
